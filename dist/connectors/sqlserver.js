...
class SQLServerConnector extends base_1.BaseDatabaseConnector {
    pool = null;
    constructor(config) {
        super(config);
    }
    async connect() {
        ...
        this.pool = await mssql_1.default.connect(poolConfig);
    }
    async disconnect() {
        if (this.pool) {
    ...
    `;
        ...
        request.input('tableName', mssql_1.default.VarChar, tableName);
        request.input('schema', mssql_1.default.VarChar, schema);
    ...
    `;
        ...
        request2.input('tableName', mssql_1.default.VarChar, tableName);
        request2.input('schema', mssql_1.default.VarChar, schema);
        const indexesResult = await request2.query(indexesQuery);
        const indexes = indexesResult.recordset.map((idx) => ({
            name: idx.name,
    ...
    `;
        ...
        request3.input('tableName', mssql_1.default.VarChar, tableName);
        request3.input('schema', mssql_1.default.VarChar, schema);
        const fkResult = await request3.query(foreignKeysQuery);
        const foreignKeys = fkResult.recordset.map((fk) => ({
            name: fk.name,
            column: fk.column,
